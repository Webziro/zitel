...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($title_err) && empty($content_err) && empty($image_err)) {
        $sql = "UPDATE blog_posts SET title = ?, content = ?, image_path = ?, category = ?, status = ?, publish_date = ? WHERE id = ?";
...
<body>
    ...
    <div class="container mt-4">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"] . "?id=" . $id); ?>" method="post" enctype="multipart/form-data">
            ...
            <div class="mb-3" id="publish_date_container" style="display: <?php echo $status == 'scheduled' ? 'block' : 'none'; ?>;">
